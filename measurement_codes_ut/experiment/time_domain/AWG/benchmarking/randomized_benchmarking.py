...
class RandomizedBenchmarking:
    ...
    def prepare(self, tdm, note):
        ...
        meas.add(Square(amplitude=note.cavity_readout_amplitude,
                 duration=note.readout_pulse_length), readout_port, copy=False)
        meas.add(Acquire(duration=note.readout_pulse_length), acq_port, copy=False)
