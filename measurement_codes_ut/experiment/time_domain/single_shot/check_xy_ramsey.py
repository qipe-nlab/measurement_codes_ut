...
class CheckXYRamsey(object):
    ...
    def __init__(self, num_shot=1000, repetition_margin=200e3, min_duration=10, max_duration=1000, hand_detune=1e6, num_sample=51):
        self.dataset = None
    ...
    def analyze(self, dataset, note, savefig=False, savepath="./fig"):
        ...
        response = dataset.data['readout_acquire']['values'].reshape(
            2, self.len_data, self.num_shot, 2)  # phase, delay, IQ, shot
        ...
        xdata = response[0]
        ydata = response[1]
        paulix = np.zeros(self.len_data)
        pauliy = np.zeros(self.len_data)
        for i in range(self.len_data):
            pred_x = np.mean([get_pred(data, note.measurement_axis, note.readout_assignment_border, note.readout_g_direction) for data in xdata[i]])
            pred_y = np.mean([get_pred(data, note.measurement_axis, note.readout_assignment_border, note.readout_g_direction) for data in ydata[i]])
            paulix[i] = pred_x
            pauliy[i] = pred_y
