...
class OptimizeHalfPiDRAG(object):
    ...
    def analyze(self, dataset, note, savefig=True, savepath="./fig"):
        ...
        len_data = len(drag_range)
        response = dataset.data['readout_acquire']['values'].reshape(2, len(self.drag_range), self.num_shot, 2)
        pm_label = ["+", "-"]
        ...
        for key, val in response_dict.items():
            ...
            for i in range(len_data):
                pred = np.mean([get_pred(data, note.measurement_axis, note.readout_assignment_border, note.readout_g_direction) for data in response_dict[key][i]])
                component[i] = pred
