...
class OptimizeHalfPiAmp(object):
    ...
    input_parameters = [
        ...
        "pi_pulse_power",
        "pi_pulse_length",
        "half_pi_pulse_ft_power",
        "half_pi_pulse_ft_drag",
        "half_pi_pulse_ft_length",
        "half_pi_pulse_ft_ramp",
    ...
    def take_data(self, tdm, calibaration_note):
        ...
        half_pi_pulse_length = note.half_pi_pulse_ft_length

        amp_range = self.amp_range
        phase_list = [0, np.pi]

        amplitude = Variable("amplitude", self.amp_range, "V")
        drag_amplitude = Variable("drag_amplitude", 1j*self.amp_range*note.half_pi_pulse_ft_drag, "V")
        ...
        with rx90.align(qubit_port, 'left'):
            rx90.add(FlatTop(RaisedCos(amplitude=amplitude, duration=note.half_pi_pulse_ft_ramp), top_duration=int(half_pi_pulse_length)),
                qubit_port, copy=False)
            rx90.add(Deriviative(FlatTop(RaisedCos(amplitude=drag_amplitude, duration=note.half_pi_pulse_ft_ramp), top_duration=int(half_pi_pulse_length))), qubit_port, copy=False)
    ...
    def analyze(self, dataset, note, savefig=True, savepath="./fig"):
        ...
        experiment_note.half_pi_pulse_ft_power = self.hpi_center
