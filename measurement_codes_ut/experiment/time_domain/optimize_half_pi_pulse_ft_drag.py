...
class OptimizeHalfPiDRAG(object):
    ...
    def take_data(self, tdm, calibaration_note):
        ...
        half_pi_pulse_power = note.half_pi_pulse_ft_power
        ...
        with rx90.align(qubit_port, 'left'):
            rx90.add(FlatTop(RaisedCos(amplitude=half_pi_pulse_power, duration=note.half_pi_pulse_ft_ramp), top_duration=int(half_pi_pulse_length)),
                qubit_port, copy=False)
            rx90.add(Deriviative(FlatTop(RaisedCos(amplitude=drag, duration=note.half_pi_pulse_ft_ramp), top_duration=int(half_pi_pulse_length))), qubit_port, copy=False)
    ...
    def analyze(self, dataset, note, savefig=True, savepath="./fig"):
        ...
        experiment_note.half_pi_pulse_ft_drag = self.drag_center
